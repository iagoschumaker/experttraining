...
// Entradas manuais de RECEITA (separado de Mensalidades)
// ============================================================================
...
import {
  TrendingUp, Plus, ArrowUpRight, CheckCircle, Trash2, Edit,
...
export default function ReceiveisPage() {
  ...
  const [editForm, setEditForm] = useState({ description: '', amount: '', date: '', categoryId: '', paymentMethod: '', notes: '', status: '' })
  ...
  const loadEntries = useCallback(async () => {
    try {
      setLoading(true)
      const params = new URLSearchParams({ page: String(page), limit: '50', type: 'RECEITA' })
  ...
  const resetForm = () => setForm({
    ...
    date: new Date().toISOString().split('T')[0],
    paymentMethod: '', notes: '', isPaid: false,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await res.json()
      if (result.success) { toast.success('Recebível criado!'); setDialogOpen(false); resetForm(); loadEntries() }
  ...
  const handleMarkPaid = async (id: string) => {
    try {
      ...
      const result = await res.json()
      if (result.success) { toast.success('Marcado como recebido!'); loadEntries() }
  ...
  // Totais rápidos
  const totalRecebido = entries.filter(e => e.status === 'PAID').reduce((s, e) => s + e.amount, 0)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{total} entradas manuais</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4">
          <p className="text-xs text-emerald-400/70 mb-1 uppercase tracking-wide">Recebido</p>
          <p className="text-xl font-bold text-emerald-400">{fmt(totalRecebido)}</p>
        </div>
        ...
        <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4">
          <p className="text-xs text-red-400/70 mb-1 uppercase tracking-wide">Vencido</p>
          <p className="text-xl font-bold text-red-400">{fmt(totalVencido)}</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <Card className="bg-card border-border"><CardContent className="pt-4">
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="text-xs mt-1">Clique em "Novo Recebível" para adicionar</p>
          </div>
        </CardContent></Card>
      ) : (
        sortedKeys.map(key => {
            ...
            <Card key={key} className="bg-card border-border">
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-emerald-400 font-semibold text-sm">+{fmt(monthTotal)}</span>
                </div>
              </CardHeader>
              <CardContent>
                  ...
                  {monthEntries.map(entry => (
                    <div key={entry.id} className="p-2.5 rounded-lg hover:bg-muted/50">
                      <div className="flex items-center gap-2 min-w-0">
                        <div className="p-1.5 rounded-md flex-shrink-0 bg-emerald-500/10">
                        ...
                        <span className="text-sm font-bold flex-shrink-0 text-emerald-400">
                          +{fmt(entry.amount)}
                        </span>
                      </div>
            ...
            <div className="border rounded-lg p-3 space-y-3">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={form.isRecurrent}
                  ...
                  className="rounded" />
                <span className="text-sm flex items-center gap-1"><RefreshCw className="h-3.5 w-3.5" /> Recorrente</span>
              </label>
              {form.isRecurrent && (
                <div className="grid grid-cols-2 gap-3">
                  <div>
